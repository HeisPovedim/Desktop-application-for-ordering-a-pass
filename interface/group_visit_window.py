...
class GroupVisit(QMainWindow):
  def __init__(self, selection_window):
    ...
    # Настройка окна
    self.setWindowTitle("Групповое посещение")
    ...
    self.list_of_visitors = ListOfVisitors()
  ...
  def initGUI(self):
    ...
    grid.addWidget(self.list_of_visitors, 1, 1)
  ...
  def make_application(self):
    ...
    if self.list_of_visitors.file_document is None or self.list_of_visitors.file_document == "":
      blank_fields.append("Прикрепите список посетителей")
      self.list_of_visitors.file_document = None
      form_valid = False
    ...
    if not form_valid:
      ...
      if self.list_of_visitors.file_document:
        ...
        try:
          with open(self.list_of_visitors.file_document, 'rb') as file:
            data = file.read()
          with open(file_path, 'ab') as file:
            file.write(data)
          print("Файлы успешно сохранены.")
